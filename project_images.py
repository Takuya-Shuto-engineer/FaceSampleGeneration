...
def project_image(proj, src_file, dst_dir, tmp_dir, npy_dir, video=False):
    ...
    misc.save_image_grid(proj.get_images(), filename, drange=[-1,1])
    filename = os.path.join(npy_dir, os.path.basename(src_file)[:-4] + '.npy')
...
def main():
    ...
    parser.add_argument('npy_dir', help="Directory of latent representation")
    ...
    for src_file in src_files:
        project_image(proj, src_file, args.dst_dir, args.tmp_dir, args.npy_dir, video=args.video)
        if args.video:
            render_video(
