...
def main():
    ...
    parser.add_argument('--layer', default=9, help='Final layer for perceptual model', type=int)
    parser.add_argument('--model', default="vgg16", help='Model for perceptual model')
    ...
    perceptual_model = PerceptualModel(args.image_size, layer=args.layer, batch_size=args.batch_size)
    perceptual_model.build_perceptual_model(generator.generated_image, args.model)
    ...
    for images_batch in tqdm(split_to_batches(ref_images, args.batch_size), total=len(ref_images)//args.batch_size):
        ...
        for img_array, dlatent, img_name in zip(generated_images, generated_dlatents, names):
            ...
            img.save(os.path.join(png_directory, f'layer{args.layer}.png'), 'PNG')
            np.save(os.path.join(npy_directory, f'layer{args.layer}.npy'), dlatent)
